...
class _AutoSearchScreenState extends State<AutoSearchScreen> {
  ...
  Future<void> pickMultipleImages() async {
    final results = await FilePicker.platform.pickFiles(
      allowMultiple: true, // 여러 파일을 선택할 수 있도록 설정
    ...
    for (var image in images) {
      String fileName = basename(image.path); // 'path' 패키지를 사용해 파일 이름 추출
      String filePath = image.path;

      // Firebase Storage에 파일 업로드
      String downloadURL = await widget.storage.uploadFile(filePath, fileName);
      print('Download URL: $downloadURL');
    }

      // 개발자가 원하는 대로 images 리스트와 count 변수를 사용할 수 있습니다.
      // 예를 들어, 상태를 업데이트하거나 UI에 표시할 수 있습니다.
      setState(() {
        _images = images; // _imageFiles는 이미지 파일 리스트를 저장하기 위한 상태 변수
        _imageCount = count; // _imageCount는 선택된 이미지의 개수를 저장하기 위한 상태 변수
      });
...
///////////////////////////////////////////////////////////////
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => ImageSearchLoad(imageFile: _images),
                      ),
                    );*/
...
/*
                    Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => ImageSearchLoad(imageFile: _image)),
                    );
*/
